...
const submitRecord = async function () {
  if (data.expense == '') {
    ...
    if (data.amount == 0) {
      ...
      const expenseRef = query(collection(db, "expense"), where('expense_id', '>', 0));
      ...
      await setDoc(doc(db, "expense", maxId.toString()), postData);
      data.errorShow = true;
  ...
  <Transition>
    <div class="container" id="newExpense">
      ...
      <div id="daily-expense">
        <button class="button button-outline" @click="fillDaily('Cf sáng', 20000, '')"><span
            class="material-symbols-outlined">
            local_cafe
          </span> 20k </button>
        <button class="button button-outline" @click="fillDaily('Ăn sáng', 17000, 'Bánh mì')"><span
            class="material-symbols-outlined">
            bakery_dining
          </span> 17k </button>
        <button class="button button-outline" @click="fillDaily('Ăn sáng', 20000, '')"><span
            class="material-symbols-outlined">
            bakery_dining
          </span> 20k </button>
        <button class="button button-outline" @click="fillDaily('Ăn trưa', 40000, '')"><span
            class="material-symbols-outlined">
            ramen_dining
          </span> 40k </button>
      </div>
